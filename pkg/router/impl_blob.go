...
func (s *httpService) blobRequestHandler(resp http.ResponseWriter, req *http.Request, details interface{}) {
	...
	mes := blob.BLOBMessage{
		BLOBBaseMessage: blob.BLOBBaseMessage{
			Req:      req,
			Resp:     resp,
			WSID:     istructs.WSID(wsid),
	...
	if !s.blobProcBus.Submit(0, 0, mes) {
		resp.WriteHeader(http.StatusServiceUnavailable)
		resp.Header().Add("Retry-After", strconv.Itoa(coreutils.DefaultRetryAfterSecondsOn503))
		close(mes.DoneChan)
		return
	}
	<-mes.DoneChan
...
func (s *httpService) blobReadRequestHandler() http.HandlerFunc {
	return func(resp http.ResponseWriter, req *http.Request) {
		...
		blobReadDetails := blob.BLOBReadDetails{
...
func (s *httpService) blobWriteRequestHandler() http.HandlerFunc {
	return func(resp http.ResponseWriter, req *http.Request) {
		...
		if len(queryParamName) > 0 {
			s.blobRequestHandler(resp, req, blob.BLOBWriteDetailsSingle{
				Name:     queryParamName,
				MimeType: queryParamMimeType,
			})
		} else {
			s.blobRequestHandler(resp, req, blob.BLOBWriteDetailsMultipart{
				Boundary: boundary,
			})
		}
	}
...
func headerOrCookieAuth(rw http.ResponseWriter, req *http.Request) (principalToken string) {
	...
	for _, c := range req.Cookies() {
		if c.Name == coreutils.Authorization {
			...
			if err != nil {
				coreutils.WriteTextResponse(rw, "failed to unescape cookie '"+c.Value+"'", http.StatusBadRequest)
				return ""
			}
...
func getBlobParams(rw http.ResponseWriter, req *http.Request) (name, mimeType, boundary string, ok bool) {
	badRequest := func(msg string) {
		coreutils.WriteTextResponse(rw, msg, http.StatusBadRequest)
	}
