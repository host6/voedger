...
func (f *implIFederation) UploadTempBLOB(appQName appdef.AppQName, wsid istructs.WSID, blobReader iblobstorage.BLOBReader, duration iblobstorage.DurationType,
	...
	if !slices.Contains(resp.ExpectedHTTPCodes(), resp.HTTPResp.StatusCode) {
		return "", fmt.Errorf("status %d, expected %v", resp.HTTPResp.StatusCode, resp.ExpectedHTTPCodes())
	}
...
func (f *implIFederation) UploadBLOB(appQName appdef.AppQName, wsid istructs.WSID, blobReader iblobstorage.BLOBReader,
	...
	if !slices.Contains(resp.ExpectedHTTPCodes(), resp.HTTPResp.StatusCode) {
		return istructs.NullRecordID, fmt.Errorf("status %d, expected %v", resp.HTTPResp.StatusCode, resp.ExpectedHTTPCodes())
	}
...
func getSysError(httpResp *coreutils.HTTPResponse) (sysError coreutils.SysError, hasSysError bool, err error) {
	...
	if len(httpResp.Body) == 0 || httpResp.HTTPResp.StatusCode == http.StatusOK {
		return sysError, false, nil
	}
	m := map[string]interface{}{}
	if err := json.Unmarshal([]byte(httpResp.Body), &m); err != nil {
		return sysError, false, fmt.Errorf("failed to unmarshal response body: %w. Body:\n%s", err, httpResp.Body)
	}
	var sysErrorIntf interface{}
	...
	return sysError, hasSysError, nil
}
