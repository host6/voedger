...
- Keep recovery demand-driven and key command-processor state by application QName and partition ID; invalidate only the affected partition after a storage or sync-actualizer failure.
- Encapsulate partition lifecycle in a dedicated partition-recovery manager backed by one keyed map whose entries contain recovered partition data, the recovering marker, and the retained recovery error; keep synchronization, worker tracking, test hooks, invalidation, and shutdown in the same manager.
...
- [x] update: [command/provide.go](../../../../../pkg/processors/command/provide.go)
  ...
  - update: the command pipeline to resolve partition readiness before request stages such as rate limiting and authorization
  ...
  - update: partition restart scheduling to invalidate only the affected application partition instead of every partition of the application
